...
### Non-Goals
- **Code/HTML/SVG execution** — Loom must never execute file content; code, HTML, and SVG are shown as source (render-state `SOURCE`) and are never interpreted/executed (Law 1). *(SVG additionally shows a passive, script-inert `<img>` preview above its source per FR-7; this is decoded display, not execution.)*
- **Human participation in chat** — the chat is the agents' communication channel; the human watches but does not post into it.
- **Full image decoding for unsupported/oversize files** — natively-decodable image types (PNG, JPEG, GIF, WebP, AVIF, BMP, ICO) up to 10MB decode and render as real previews; files that are an unsupported type or exceed 10MB fall back to a safe placeholder (see Section 11, OQ-5). **TIFF** is mapped to the image kind and base64-encoded but is **not natively decodable by the renderer's Chromium engine** (no transcoding dependency ships), so its preview is best-effort and may not display (FR-10 TIFF caveat). **SVG is not a preview type** — it is a `SOURCE` render-state that additionally shows a passive `<img>` preview above its source (FR-7); it is not counted among the decoded-preview image types here.
...
- **FR-7** — `.svg` files MUST be treated as code source: the render-state MUST be **`SOURCE`** (never **`PREVIEW`**), the syntax-highlighted SVG markup MUST be shown, and the SVG MUST **NOT be interpreted/executed as live markup** (no script, no external fetches, no DOM rendering of the SVG itself). *(Reconciliation — supplementary preview):* the shipped Viewer additionally renders a **sandboxed, decoded `<img>` preview above the source** (the SVG bytes are base64-encoded into a `data:image/svg+xml` URI and shown via a passive `<img>`, which cannot run script). This supplementary `<img>` preview does **not** change the `SOURCE` render-state and does **not** make the SVG "live": the authoritative representation remains the source view, and no SVG-authored script or navigation can run. *(This relaxes the earlier blanket "MUST NOT be rendered as images" to permit only the passive, script-inert `<img>` preview that ships; see FR-10's SVG note and the Non-Goal below.)*
- **FR-8** — The viewer MUST NOT execute any file content; markdown renders, but code, HTML, and SVG are shown as source (render-state **`SOURCE`**). *(SVG additionally shows a passive, script-inert `<img>` preview above its source per FR-7 — this is decoded display, not execution, and does not weaken the no-execution guarantee.)* (Law 1)
- **FR-9** — Every file type MUST render as something visible; unknown/binary files MUST display a metadata placeholder. (Law 2)
- **FR-10** — Images MUST be handled as a **first-class viewer render-state** (`PREVIEW`): for **supported image types** (PNG, JPEG, GIF, WebP, AVIF) **at or below the 10MB cap**, the Viewer MUST display the **decoded image** (the main process base64-encodes the bytes into a `data:` URI and the Viewer renders a real `<img>`). For files that are an **unsupported type or exceed 10MB**, the Viewer MUST fall back to a **safe placeholder** (labeled e.g. "{type} · safe preview", shown as a checkerboard card). *(Reconciled in v0.4; image decoding now SHIPS — see OQ-5.)* **(decoded-preview and placeholder-fallback behavior verified against `src/main/sandbox.ts` and `src/renderer/components/Viewer.tsx`)**
  - **BMP / ICO** are also mapped to the `image` kind (`PREVIEW`) and base64-encoded; these are decoded natively by the renderer's Chromium engine. **(Reconciled with `IMAGE_EXTS` in `src/shared/dispatch.ts`.)**
  - **TIFF caveat:** `.tiff` is mapped to the `image` kind and the main process base64-encodes it into a `data:image/tiff` URI, but the renderer (Electron 33 / Chromium ~130) does **NOT** natively decode TIFF in an `<img>` and Loom ships **no transcoding dependency** (no sharp/jimp/canvas). A TIFF preview is therefore **best-effort and may fail to display** (showing the `<img>` alt text rather than a rendered image). TIFF MUST NOT be relied upon as a guaranteed "real preview." *(See OQ-5; tracked as a known limitation.)*
  - **SVG is NOT a `PREVIEW` render-state.** Per FR-7/FR-8, `.svg` is dispatched as a **`SOURCE`** render-state (kind `svg`, with the safety banner). As a convenience, the Viewer additionally shows a **decoded `<img>` preview *above* the SVG source** (the main process supplies `content.imageData` for SVG as well); this preview is supplementary to — not a replacement for — the source view, and the render-state badge remains **SOURCE**, not **PREVIEW**. See FR-7 for the reconciliation of "MUST NOT be rendered as images." **(Reconciled with `src/shared/dispatch.ts` and the SOURCE branch in `src/renderer/components/Viewer.tsx`.)**
...
#### Accessibility
- **FR-54** — All interactive controls — roster chips, channel tabs, the theme toggle, the inbox "← channels" back control, and Explorer file rows — MUST be **operable by keyboard** with proper interactive semantics (not click-only `div`/`span` handlers) and a **visible focus indicator**. *(Implemented: roster chips, channel tabs, and Explorer file rows are real keyboard-operable controls (native `<button>` for chips/tabs; `role=treeitem` rows with roving tabindex and APG tree keyboard navigation) with a shared `:focus-visible` ring; WCAG 2.2 AA SC 2.1.1, 2.4.7.)* **(inferred from design prototype)**
...
- **NFR-12 (Accessibility — target WCAG 2.2 AA)** — The user interface MUST meet WCAG 2.2 Level AA. **Keyboard + focus (SC 2.1.1, 2.4.7) is now implemented** (see FR-54); the remaining sub-criteria below are partially addressed and full AA conformance has not yet been independently audited. At minimum:
  ...
  **(keyboard + focus implemented per FR-54; the remaining sub-criteria above are partially addressed and have not yet been independently audited for full WCAG 2.2 AA conformance)**
...
### Assumptions
- Image preview ships now with real (Chromium-native) decoding for supported types (PNG, JPEG, GIF, WebP, AVIF, BMP, ICO) up to 10MB; a safe placeholder is the fallback for unsupported or oversize (>10MB) files (see OQ-5). **TIFF** is base64-encoded but is not natively decoded by the renderer (no transcoding dependency ships), so its preview is best-effort and may not display (FR-10 TIFF caveat). **SVG** is a `SOURCE` render-state (FR-7), not a decoded-preview type, though it additionally shows a passive `<img>` preview above its source.
...
- **AC-3c** — An `.svg` file's render-state is **`SOURCE`** (badge SOURCE, not PREVIEW) with the SVG markup shown as highlighted source and never interpreted/executed; the supplementary passive `<img>` preview shown above the source does not change this render-state. (FR-7)
...
- **OQ-5 (Image preview — RESOLVED; decoding now SHIPPED, with two caveats)** — The earlier phase-2 vs phase-5 inconsistency is resolved and image decoding has since shipped: for natively-decodable types (PNG, JPEG, GIF, WebP, AVIF, BMP, ICO) at or below the 10MB cap, the Viewer renders the **decoded image** in the `PREVIEW` render-state (FR-10, FR-40); the **safe placeholder** is now the fallback only for unsupported or oversize (>10MB) files. **Caveat — TIFF:** `.tiff` is mapped to the image kind and base64-encoded, but the renderer (Electron 33 / Chromium ~130) does not natively decode TIFF in an `<img>` and no transcoding dependency ships, so a TIFF preview is best-effort and may show the `<img>` alt text instead of an image (FR-10 TIFF caveat). **Caveat — SVG is not a `PREVIEW` type:** `.svg` is a `SOURCE` render-state (FR-7); the Viewer shows a passive, script-inert `<img>` preview *above* its source, but the render-state badge stays **SOURCE**, not **PREVIEW**. **Note (badge styling):** because the `PREVIEW` badge previously reused the green "rendered" visual family when only a placeholder was shown, this no longer misrepresents natively-decoded images; the placeholder fallback (and the best-effort TIFF case) still warrants a badge-styling review so it does not imply true rendering.
- **OQ-6 (Terminal pane — RESOLVED; SHIPPED as a human-only bottom dock)** — Earlier descriptions named a fourth "terminal" pane (project memory: "explorer/viewer/chat/terminal"), absent from the original design. It has since shipped — **not** as one of the three side-by-side content panes, but as a **human-only bottom-dock terminal** below the 3-pane body, toggled with **Ctrl+`** (`toggleTerminal`). It is built on **@xterm/xterm** in the renderer and **node-pty** in the main process, driven over `loom:terminal:*` IPC. The dock is **MCP-invisible (no agent surface)**: agents cannot see or drive it, so it does not weaken Law 1's no-execution guarantee for rendered file/message content — it is a convenience for the human observer only.
